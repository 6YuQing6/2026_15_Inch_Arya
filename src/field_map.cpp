...
Circle::Circle() : name(""), cx(0), cy(0), radius(0) {}
Circle::Circle(const std::string &n, double x, double y, double r)
    : name(n), cx(x), cy(y), radius(r) {}
...
bool Circle::intersectsSegment(double x1, double y1, double x2, double y2) const {
    if (contains(x1, y1) || contains(x2, y2)) return true;
...
void FieldMap::addObstacle(const Circle &c) {
...
bool FieldMap::lineIntersectsObstacles(double x1, double y1, double x2, double y2) const {
    ...
    for (const auto &c : m_circles) {
        if (c.intersectsSegment(x1,y1,x2,y2)) return true;
    }
    return false;
}
...
void FieldMap::populateStandardField() {
    ...
    struct Entry { const char *name; double x1,y1,x2,y2,long_cm,short_cm,margin; } entries[] = {
        ...
        {"LongGoal1", 63.0, 120.0, -63.0, 120.0, 124.0, 14.0, 3.0},
        {"LongGoal2", 63.0, -120.0, -63.0, -120.0, 124.0, 14.0, 3.0},
        // MiddleSquare removed - replaced by circle below
        {"BluePark", 135.0, 0.0, 183.0, 0.0, 48.0, 55.0, 3.0},
        {"RedPark", -135.0, 0.0, -183.0, 0.0, 48.0, 55.0, 3.0},
    ...
    // Center structure as circle: 27cm physical radius + 10cm margin = 37cm
    addObstacle(Circle("MiddleCircle", 0.0, 0.0, 46.0));
