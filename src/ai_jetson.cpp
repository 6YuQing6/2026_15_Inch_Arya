...
/*---------------------------------------------------------------------------*/
/** @brief  Set localizer position to send to Jetson                         */
/*---------------------------------------------------------------------------*/
void
jetson::set_localizer_position(float x_cm, float y_cm, float heading_deg) {
    poslock.lock();
    localizer_x_cm = x_cm;
    localizer_y_cm = y_cm;
    localizer_heading_deg = heading_deg;
    poslock.unlock();
}
...
jetson::request_map() {
    ...
    if( state == jetson_state::kStateSyncWait1 ) {
      // Send message to jetson with localizer position data
      // Format: 4 sync bytes + 12 bytes of binary position data (3 floats)
      // Using binary sync bytes that match what Jetson already expects
      // serial1 is the second USB cdc channel dedicated to user programs
      //
      FILE *fp = fopen("/dev/serial1", "wb");

      // Send 4-byte binary sync header (same sync as Jetson->V5 packets)
      static uint8_t sync[] = { 0xC5, 0x5C, 0x3A, 0xA3 };  // Unique sync for V5->Jetson position
      fwrite( sync, 1, 4, fp );
      
      // Get current localizer position (thread-safe)
      poslock.lock();
      float x = localizer_x_cm;
      float y = localizer_y_cm;
      float h = localizer_heading_deg;
      poslock.unlock();
      
      // Convert to meters for Jetson (Jetson uses meters internally)
      float x_m = x / 100.0f;
      float y_m = y / 100.0f;
      
      // Send binary position data: x(float), y(float), heading(float) = 12 bytes
      fwrite( &x_m, sizeof(float), 1, fp );
      fwrite( &y_m, sizeof(float), 1, fp );
