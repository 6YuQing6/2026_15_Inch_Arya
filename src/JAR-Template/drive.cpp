...
void Drive::drive_with_voltage(float leftVoltage, float rightVoltage){
  // Motor groups are physically swapped: DriveL=physical right, DriveR=physical left
  // Also negated to flip front/back direction
  DriveR.spin(fwd, -leftVoltage, volt);   // DriveR is physical left
  DriveL.spin(fwd, -rightVoltage, volt);  // DriveL is physical right
}
...
float Drive::get_left_position_in(){
  // DriveR is physical left, negate because we drive with negative voltage to go forward
...
float Drive::get_right_position_in(){
  // DriveL is physical right, negate because we drive with negative voltage to go forward
...
void Drive::turn_to_angle(float angle, float turn_max_voltage, float turn_settle_error, float turn_settle_time, float turn_timeout, float turn_kp, float turn_ki, float turn_kd, float turn_starti){
  ...
  while( !turnPID.is_settled() ){
    ...
    drive_with_voltage(-output, output);  // Flipped to fix turn direction
    task::sleep(10);
  }
}
...
void Drive::drive_distance(float distance, float heading, float drive_max_voltage, float heading_max_voltage, float drive_settle_error, float drive_settle_time, float drive_timeout, float drive_kp, float drive_ki, float drive_kd, float drive_starti, float heading_kp, float heading_ki, float heading_kd, float heading_starti){
  ...
  while(drivePID.is_settled() == false){
    ...
    drive_with_voltage(drive_output-heading_output, drive_output+heading_output);  // Flipped heading for turn direction
    task::sleep(10);
  }
}
...
void Drive::left_swing_to_angle(float angle, float swing_max_voltage, float swing_settle_error, float swing_settle_time, float swing_timeout, float swing_kp, float swing_ki, float swing_kd, float swing_starti){
  ...
  while(swingPID.is_settled() == false){
    ...
    output = clamp(output, -turn_max_voltage, turn_max_voltage);
    // DriveR is physical left, spin it for left swing
    DriveR.spin(fwd, -output, volt);
    DriveL.stop(hold);  // DriveL is physical right, hold it
    task::sleep(10);
  }
}
...
void Drive::right_swing_to_angle(float angle, float swing_max_voltage, float swing_settle_error, float swing_settle_time, float swing_timeout, float swing_kp, float swing_ki, float swing_kd, float swing_starti){
  ...
  while(swingPID.is_settled() == false){
    ...
    float output = swingPID.compute(error);
    output = clamp(output, -turn_max_voltage, turn_max_voltage);
    // DriveL is physical right, spin it for right swing
    DriveL.spin(fwd, -output, volt);
    DriveR.stop(hold);  // DriveR is physical left, hold it
    task::sleep(10);
  }
}
...
void Drive::control_arcade(){
  ...
  float turn = -deadband(controller(primary).Axis1.value(), 5);  // Negated to fix turn direction
  // Motor groups are physically swapped: DriveL=physical right, DriveR=physical left
  // Also negated to flip front/back direction
  DriveR.spin(fwd, -to_volt(throttle+turn), volt);  // Physical left = throttle+turn
  DriveL.spin(fwd, -to_volt(throttle-turn), volt);  // Physical right = throttle-turn
}
...
void Drive::control_tank(){
  ...
  // Motor groups are physically swapped: DriveL=physical right, DriveR=physical left
  // Also negated for flipped front/back
  DriveR.spin(fwd, -to_volt(leftthrottle), volt);   // Physical left
  DriveL.spin(fwd, -to_volt(rightthrottle), volt);  // Physical right
}
